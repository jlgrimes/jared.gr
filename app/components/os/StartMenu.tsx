...
export const StartMenu: React.FC<StartMenuProps> = ({ isOpen, onClose }) => {
  ...
  const handleAppClick = (app: any) => {
    openWindow({ id: app.id, title: app.name, icon: app.icon, content: app.content });
    onClose();
  };
        ...
        >
          ...
          <div className='flex-1 px-8 py-4 overflow-y-auto'>
            <div className='flex justify-between items-center mb-4'>
              <h3 className='text-sm font-semibold text-gray-800 dark:text-white'>Pinned</h3>
            ...
            <div className='mt-8'>
               <div className='flex justify-between items-center mb-4'>
                  <h3 className='text-sm font-semibold text-gray-800 dark:text-white'>Recommended</h3>
                  <button className='text-xs text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white bg-white/50 dark:bg-white/10 px-2 py-1 rounded shadow-sm hover:shadow transition-all'>
                     More &gt;
                  </button>
               </div>
                  ...
                  {desktopApps.slice(0, 2).map((app) => (
                     ...
                     >
                        ...
                        <div className='flex flex-col'>
                           <span className='text-xs font-medium text-gray-800 dark:text-gray-200'>{app.name}</span>
                           <span className='text-[10px] text-gray-500 dark:text-gray-400'>Recently added</span>
                        </div>
                     </div>
                  ))}
               </div>
