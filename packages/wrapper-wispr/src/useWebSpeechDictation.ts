 ...
 */
export const useWebSpeechDictation = (): Dictation => {
  ...
  // ctx/stream are absent on the synthesized-waveform path, where there's no real mic open.
  ...
  const teardownAudio = React.useCallback(() => {
    ...
    // The synthesized-waveform path has no context or stream to release.
